    ...
    params: {
      ...
      arguments: {
        options: [
          ...
          {
            name: 'Hybrid Solution C',
            description: 'Mixed approach',
            criteria: { cost: 7, scalability: 7, security: 8, performance: 8 }
          }
        ],
        criteria: [
          { name: 'cost', weight: 0.3, description: 'Total cost of ownership' },
          { name: 'scalability', weight: 0.3, description: 'Ability to scale' },
          { name: 'security', weight: 0.25, description: 'Security features' },
          { name: 'performance', weight: 0.15, description: 'System performance' }
        ]
      }
    }
  };
...
// Test 5: Sequential thinking for complex problem
setTimeout(() => {
  console.log('\n🧠 Test 5: Sequential thinking for complex problem...');
  const sequentialThinkingRequest = {
    jsonrpc: '2.0',
    id: 5,
    method: 'tools/call',
    params: {
      ...
      arguments: {
        thought: 'I need to analyze why our customer retention rate has dropped from 85% to 70% over the past 6 months. Let me start by identifying the key factors that could contribute to this decline.',
        thoughtNumber: 1,
        totalThoughts: 5,
        nextThoughtNeeded: true,
        confidence: 0.8,
        tags: ['customer-retention', 'business-analysis']
      }
    }
  };
  
  server.stdin.write(JSON.stringify(sequentialThinkingRequest) + '\n');
}, 4000);
